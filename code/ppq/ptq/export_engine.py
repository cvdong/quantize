...
LastEditTime: 2022-11-17 17:48:44
FilePath: /Inference/ppq/export_engine.py
Description: 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--onnx', type=str, default='Quantized.onnx')
    parser.add_argument('--qparam_json', type=str, default='Quantized.json')
    parser.add_argument('--engine', type=str, default='ppqq.engine')
    arg = parser.parse_args()

    # build_engine(arg.onnx, arg.qparam_json, arg.engine)
    
    if not os.path.exists(arg.engine): 
        build_engine(
            onnx_file=arg.onnx, 
            engine_file=arg.engine, int8=True, 
            int8_scale_file=arg.qparam_json)
    
    print("\033[1;32mgenerate %s\033[0m" % arg.engine)
    
    Benchmark(arg.engine)
    Profiling(arg.engine)